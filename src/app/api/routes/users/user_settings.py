...
async def delete_user(
    ...
    if user is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail={"reason": "invalid password"})
...
async def update_password(
    ...
    try:
        credentials = OAuth2PasswordRequestForm(username=user.email, password=user_update.password)
        user = await user_manager.authenticate(credentials)
        if user is None:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail={"reason": "invalid password"})
