...
class UserService:
    ...
    async def get_artist_likes(self, uow: UnitOfWork, user_id: int):
        ...
        artists = [
            (await ArtistsService.get_artist_by_id(uow, like.artist_id)) for like in likes
        ]
        return artists
    ...
    async def get_user_status_like(
        self, uow: UnitOfWork, user_id: int, like_type: LikeType, object_id: int
    ) -> bool:
        async with uow:
            if like_type == LikeType.ARTIST:
                like = await uow.artist_like.filter(
                    limit=1, user_id=user_id, artist_id=object_id
                )
            elif like_type == LikeType.ARTWORK:
                like = await uow.artwork_like.filter(
                    limit=1, user_id=user_id, artwork_id=object_id
                )
            elif like_type == LikeType.FESTIVAL:
                like = await uow.festival_like.filter(
                    limit=1, user_id=user_id, festival_id=object_id
                )
