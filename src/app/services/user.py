...
class UserService:
    async def get_artwork_likes(self, uow: UnitOfWork, user_id: int):
        ...
        artworks = [(await ArtworksService().get_artwork(uow, like.artwork_id)) for like in likes]
        return artworks

    async def get_artist_likes(self, uow: UnitOfWork, user_id: int):
        likes = await self.get_user_likes(uow, user_id, LikeType.ARTIST)
        artists = [(await ArtistsService().get_artist_by_id(uow, like.artist_id)) for like in likes]
        return artists
    ...
    async def get_festival_likes(self, uow: UnitOfWork, user_id: int):
        ...
        festivals = [(await FestivalService().get_festival_by_id(uow, like.festival_id)) for like in likes]
        return festivals
