...
@collection_router.get("/artworks",
                       response_model=List[ArtworkCardSchema])
async def get_favorite_artworks(uow: UOWDep,
                                user: User = Depends(current_user)):
    likes = await UserService().get_artwork_likes(uow, user.id)
    return likes


@collection_router.get("/artists",
                       response_model=List[ArtistCardSchema])
async def get_favorite_artists(uow: UOWDep,
                               user: User = Depends(current_user)):
    likes = await UserService().get_artist_likes(uow, user.id)
    return likes


@collection_router.get("/festivals",
                       response_model=List[FestivalCardSchema])
async def get_favorite_festivals(uow: UOWDep,
                                 user: User = Depends(current_user)):
