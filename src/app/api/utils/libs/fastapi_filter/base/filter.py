...
class BaseFilterModel(BaseModel, extra="allow"):
    ...
    class Constants:  # pragma: no cover
        ...
        ordering_model_fields: List[str] = list()
        search_model_fields: List[str] = list()
    ...
    def add_filtering_fields(self, **fields):
        for field_name, field_value in fields.items():
            # Добавляем новое поле к текущему экземпляру модели
            setattr(self, field_name, field_value)

            # Обновляем аннотации класса
            self.__annotations__[field_name] = (
                type(field_value),
                Field(None, alias=None),
            )

            # Если требуется, обновляем ordering_model_fields и search_model_fields
            # if field_name not in self.Constants.ordering_model_fields:
            #     self.Constants.ordering_model_fields.append(field_name)
            # if field_name not in self.Constants.search_model_fields:
            #     self.Constants.search_model_fields.append(field_name)

    # def add_filtering_field(self, value):
    #     print(value)
    # temp = getattr(self, self.Constants.)
    # temp.append(value)
    # setattr(self, self.Constants.ordering_field_name, temp)
    # print(getattr(self, self.Constants.ordering_field_name))
    # fields = self.model_dump(exclude_none=True, exclude_unset=True)
    # fields.pop(self.Constants.ordering_field_name, None)
    # fields = self.model_dump(exclude_none=True, exclude_unset=True)
    # fields.pop(self.Constants.ordering_field_name, None)
    ...
    def validate_order_by(cls, value, field: ValidationInfo):
        ...
        for field_name_with_direction in value:
            ...
            if (
                cls.Constants.ordering_model_fields
                and field_name not in cls.Constants.ordering_model_fields
            ):
                raise ValueError(
                    f"You may only sort by: {', '.join(cls.Constants.ordering_model_fields)}"
                )
