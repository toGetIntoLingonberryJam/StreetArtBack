...
from app.modules.festivals.schemas import FestivalCreate
...
class FestivalService:
    ...
    async def create_festival(self, uow: UnitOfWork, festival_schema: FestivalCreate):
        async with uow:
    ...
    async def get_all_festival(self,
        ...
        async with uow:
            ...
            festivals = await uow.festival.get_all(offset=offset,
                                                   limit=limit,
                                                   filters=filters,
                                                   filter_by=filter_by)
            return festivals
    ...
    async def update_artwork_festival(self, uow: UnitOfWork, artwork_id: int, festival_id: int):
        async with uow:
            festival = await self.get_festival_by_id(uow, festival_id)
            try:
                artwork = await uow.artworks.edit(artwork_id, {"festival_id": festival.id})
                await uow.commit()
                return artwork
